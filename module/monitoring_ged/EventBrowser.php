...
class EventBrowser {
	...
	private function sql2xml($gedsql_count,$gedsql_result) {
		...
		$nbr=mysql_num_rows($gedsql_result);

		$this->gedq='<?xml version="1.0" encoding="UTF-8"?><ged '.substr($this->mystate,0,1).'="'.$gedsql_count.'">';

		for($i=0;$i<$nbr;$i++) {
		        $this->gedq.='<r t="'.mysql_result($gedsql_result,$i,"TYPE").'" q="'.mysql_result($gedsql_result,$i,"queue").'" o="'.mysql_result($gedsql_result,$i,"occ").'" i="'.mysql_result($gedsql_result,$i,"id").'">';
		        $this->gedq.='<u><s>'.mysql_result($gedsql_result,$i,"src").'</s></u>';
		        $this->gedq.='<t>';
		        $this->gedq.='<o s="'.mysql_result($gedsql_result,$i,"o_sec").'"></o>';
		        $this->gedq.='<l s="'.mysql_result($gedsql_result,$i,"l_sec").'"></l>';
		        $this->gedq.='<a s="'.mysql_result($gedsql_result,$i,"l_sec").'"></a>';
		        $this->gedq.='</t>';
		        $this->gedq.='<c>';
		        foreach($array_ged_packets as $val => $tab){
		        	if($tab["type"]==true)
				        $this->gedq.="<$val>".htmlspecialchars(mysql_result($gedsql_result,$i,$val),ENT_QUOTES)."</$val>";
			}
		       	$this->gedq.='</c>';
		        $this->gedq.='</r>';
	        }
	...
	public function showTable($mystate,$mytype=false,$myfilter=false) {
		...
		if($mytype) {
			...
			while ($line = mysql_fetch_array($gedsql_result1)) {
		...
		foreach($events as $event){
			...
			foreach($array_ged_packets as $val => $tab){
				if(isset($_POST[$event."_".$val]))
					$ged[str_replace("-","",$val)]=$_POST[$event."_".$val];		
                        }
