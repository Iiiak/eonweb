...
switch($_POST['option']){
	case "select" :
		$results=make_assoc(mysql_evaluate_array());
		$exist = false;
	...
	case "delete" :
		...
		if ( $result ) {
			$type = mysql_fetch_assoc(sqlrequest($database_nagios,"SELECT type,min_value FROM bp WHERE name='$_POST[bp]'"));
			if ( $type['type'] == "MIN") {
				...
				if ( !mysql_num_rows($return)) $result = sqlrequest($database_nagios,"UPDATE bp SET `is_define`='0' WHERE `name`='$_POST[bp]'");
			}
		}
