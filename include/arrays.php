...
$array_serv_system = array (
	...
	"SNMP trap traductor" => array (
        ...
        "Option : Ntop" => array (
                        "status" => "pidof -o $$ -o %PPID -x ntop",
                        "proc_act" => array (
                                "start" => "sudo /etc/init.d/ntop start",
                                "stop" => "sudo /etc/init.d/ntop stop",
                                "restart" => "sudo /etc/init.d/ntop restart",
                                "reload" => "sudo /etc/init.d/ntop reload")),
        "Option : Shinken" => array (
                        "status" => "ps -aef | grep shinken-arbiter |grep -v grep | awk '{print $2}'",
                        "proc_act" => array (
                                "start" => "sudo /etc/init.d/shinken start",
                                "stop" => "sudo /etc/init.d/shinken stop",
                                "restart" => "sudo /etc/init.d/shinken restart",
                                "check" => "sudo /etc/init.d/shinken check",
                                "verify" => "$path_shinken/bin/nagios -v -c $path_shinken/etc/nagios.cfg")),
);
